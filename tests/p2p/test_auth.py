...
@pytest.mark.skip("Should remove, no longer support plain auth")
@pytest.mark.asyncio
async def test_handshake():
    ...
    initiator_ephemeral_pubkey, initiator_nonce, _ = decode_auth_message(
        auth_msg_ciphertext[2:], auth_msg_ciphertext[:2], responder.privkey)
...
async def test_handshake_eip8():
    ...
    initiator_ephemeral_pubkey, initiator_nonce, _ = decode_auth_message(
        auth_init_ciphertext[2:], auth_init_ciphertext[:2], responder.privkey)
    ...
    responder_ephemeral_pubkey, responder_nonce = initiator.decode_auth_ack_message(
        auth_ack_ciphertext[2:], auth_ack_ciphertext[:2])
    (initiator_aes_secret,
     initiator_mac_secret,
     initiator_egress_mac,
     initiator_ingress_mac) = initiator.derive_secrets(

...
class ChainSyncer(PeerPoolSubscriber):
    ...
    def _skip_empty_bodies(self, headers: List[BlockHeader]) -> List[BlockHeader]:
        return [
            header
            for header in headers
    ...
    def _skip_empty_and_duplicated_receipts(self, headers: List[BlockHeader]) -> List[BlockHeader]:
    ...
    async def get_idle_peer(self) -> ETHPeer:
        ...
        while True:
            ...
            if idle_peers:
                return cast(ETHPeer, random.choice(idle_peers))
            else:

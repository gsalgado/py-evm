...
class DiscoveryProtocol(asyncio.DatagramProtocol):
    ...
    def send_topic_register(
        ...
        if idx >= len(topics):
            raise ValueError("Invalid topic idx: %d", idx)
    ...
    async def get_ticket(self, node: kademlia.Node, topics: List[bytes]) -> 'Ticket':
        ...
        try:
            ...
            self.logger.debug("failed to get ticket from %s, already waiting for pong", node)
            return None

        if not got_pong:
            self.logger.debug("failed to get ticket from %s", node)
            return None
